...
# EAP-TLS in pppd 2.4.9 requires the OpenSSL ENGINE API, which our OpenSSL 3.x
# is built without (no-engine — the API is deprecated in 3.x). No product uses
# PPP EAP-TLS. MSCHAP still gets DES from OpenSSL via the NEEDDES/-lcrypto path.
PPPD_MAKE_OPTS += USE_EAPTLS=
else
PPPD_MAKE_OPTS += \
	USE_CRYPT=y \
	USE_EAPTLS=
