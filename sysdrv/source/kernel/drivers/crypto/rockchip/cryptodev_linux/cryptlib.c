				...
				uint8_t *keyp, size_t keylen, int stream, int aead)
{
	...
	if (aead == 0) {
		...
		out->async.s = cryptodev_crypto_alloc_blkcipher(alg_name, CRYPTO_ALG_INTERNAL, CRYPTO_ALG_INTERNAL);
		...
		out->async.as = crypto_alloc_aead(alg_name, CRYPTO_ALG_INTERNAL, CRYPTO_ALG_INTERNAL);
