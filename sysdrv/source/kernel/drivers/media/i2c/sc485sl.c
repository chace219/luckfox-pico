...
static long sc485sl_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_QUICK_STREAM:
		...
		if (sc485sl->enable_light_ctl) {
			sc485sl->light_param.light_enable = stream;
			light_ctl_write(sc485sl->module_index, &sc485sl->light_param);
		}
	...
	case RKCIS_CMD_FLASH_LIGHT_CTRL:
		dev_info(&sc485sl->client->dev, "set flash light param\n");
		light_param = (struct rk_light_param *)arg;
		if (light_param->light_enable) {
			memcpy(&sc485sl->light_param, light_param, sizeof(struct rk_light_param));
			sc485sl->enable_light_ctl = true;
		} else {
			sc485sl->enable_light_ctl = false;
		}
		ret = light_ctl_write(sc485sl->module_index, light_param);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case RKCIS_CMD_FLASH_LIGHT_CTRL:
		...
		if (!ret)
			ret = sc485sl_ioctl(sd, cmd, light_param);
		else
			ret = -EFAULT;
		kfree(light_param);
		break;
