...
static const struct ar0822_mode *ar0822_find_mode(struct ar0822 *ar0822, int fps)
{
	const struct ar0822_mode *mode = NULL;
	const struct ar0822_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ar0822->cfg_num; i++) {
		mode = &supported_modes[i];
		if (mode->width == ar0822->cur_mode->width &&
		    mode->height == ar0822->cur_mode->height &&
		    mode->hdr_mode == ar0822->cur_mode->hdr_mode &&
		    mode->bus_fmt == ar0822->cur_mode->bus_fmt) {
...
static int ar0822_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct ar0822 *ar0822 = to_ar0822(sd);
	const struct ar0822_mode *mode = NULL;
	...
	mode = ar0822_find_mode(ar0822, fps);
	if (mode == NULL) {
	...
	case RKMODULE_SET_HDR_CFG:
		...
		for (i = 0; i < ar0822->cfg_num; i++) {
			if (w == supported_modes[i].width &&
			    ...
			    ar0822->cur_mode->bus_fmt == supported_modes[i].bus_fmt &&
...
static const struct v4l2_subdev_video_ops ar0822_video_ops = {
	...
	.s_frame_interval = ar0822_s_frame_interval,
};
...
static void ar0822_modify_fps_info(struct ar0822 *ar0822)
{
	const struct ar0822_mode *mode = ar0822->cur_mode;

	ar0822->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      ar0822->cur_vts;
}
...
static int ar0822_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		ar0822->cur_vts = ctrl->val + ar0822->cur_mode->height;
		ar0822_modify_fps_info(ar0822);
