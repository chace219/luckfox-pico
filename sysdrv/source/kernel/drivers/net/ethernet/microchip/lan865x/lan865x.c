...
#define LAN865X_RESET_ASSERT_MS		10
#define LAN865X_RESET_SETTLE_MS		50
...
static int lan865x_hw_reset(struct lan865x_priv *priv)
{
	...
	msleep(LAN865X_RESET_ASSERT_MS);
	gpiod_set_value_cansleep(priv->reset_gpio, 0);
	msleep(LAN865X_RESET_SETTLE_MS);

	return 0;
}

static int lan865x_read_u32_prop(struct device *dev, const char *name,
				 int *value)
{
	u32 tmp;
	int ret;
...
static int lan865x_apply_plca_cfg(struct lan865x_priv *priv)
{
	...
	enable = device_property_read_bool(dev, "microchip,plca-enable");
	requested = enable;

	if (!lan865x_read_u32_prop(dev, "microchip,plca-node-id", &node_id))
		requested = true;
	if (!lan865x_read_u32_prop(dev, "microchip,plca-node-count", &node_count))
		requested = true;
	if (!lan865x_read_u32_prop(dev, "microchip,plca-to-timer", &to_timer))
		requested = true;
	if (!lan865x_read_u32_prop(dev, "microchip,plca-burst-count",
				   &burst_count))
		requested = true;
	if (!lan865x_read_u32_prop(dev, "microchip,plca-burst-timer",
				   &burst_timer))
		requested = true;
	...
	if (node_id >= 0 || node_count >= 0) {
		...
		regval = (u16)ret;
		if (node_count >= 0)
			regval = (regval & ~MDIO_OATC14_PLCA_NCNT) |
				 ((node_count << 8) & MDIO_OATC14_PLCA_NCNT);
		if (node_id >= 0)
			regval = (regval & ~MDIO_OATC14_PLCA_ID) |
				 (node_id & MDIO_OATC14_PLCA_ID);

		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
				    MDIO_OATC14_PLCA_CTRL1, regval);
		if (ret)
			return ret;
	}
	...
	if (to_timer >= 0) {
		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
				    ...
				    to_timer & MDIO_OATC14_PLCA_TOT);
		if (ret)
			return ret;
	}

	if (burst_count >= 0 || burst_timer >= 0) {
		...
		regval = (u16)ret;
		if (burst_count >= 0)
			regval = (regval & ~MDIO_OATC14_PLCA_MAXBC) |
				 ((burst_count << 8) & MDIO_OATC14_PLCA_MAXBC);
		if (burst_timer >= 0)
			regval = (regval & ~MDIO_OATC14_PLCA_BTMR) |
				 (burst_timer & MDIO_OATC14_PLCA_BTMR);

		ret = phy_write_mmd(phydev, MDIO_MMD_VEND2,
				    MDIO_OATC14_PLCA_BURST, regval);
		if (ret)
			return ret;
	}
...
static int lan865x_probe(struct spi_device *spi)
{
	...
	ret = lan865x_hw_reset(priv);
	if (ret) {
		dev_err(&spi->dev, "Hardware reset setup failed: %d\n", ret);
		goto free_netdev;
	}

	priv->tc6 = oa_tc6_init(spi, netdev);
	if (!priv->tc6) {
		dev_err(&spi->dev, "OA-TC6 initialization failed\n");
		ret = -ENODEV;
		goto free_netdev;
	}
	...
	ret = lan865x_apply_plca_cfg(priv);
	if (ret) {
		dev_err(&spi->dev, "Failed to configure PLCA: %d\n", ret);
		goto oa_tc6_exit;
	}

	/* Get the MAC address from the SPI device tree node */
	if (eth_platform_get_mac_address(&spi->dev, mac_addr))
