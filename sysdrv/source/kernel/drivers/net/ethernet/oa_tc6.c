...
static int oa_tc6_check_ctrl_write_reply(struct oa_tc6 *tc6, u8 size)
{
	...
	u8 cmp_size = size - OA_TC6_CTRL_IGNORED_SIZE;
	 ...
	 * transmitted.
	 */
	if (memcmp(tx_buf, rx_buf, cmp_size)) {
		dev_err(&tc6->spi->dev,
			"ctrl write echo mismatch: TX[%*ph] RX[%*ph]\n",
			(int)cmp_size, tx_buf,
			(int)cmp_size, rx_buf);
		return -EPROTO;
	}
...
static int oa_tc6_check_ctrl_read_reply(struct oa_tc6 *tc6, u8 size)
{
	...
	if (*tx_buf != *rx_buf) {
		dev_err(&tc6->spi->dev,
			"ctrl read echo mismatch: TX[%*ph] RX_echo[%*ph] RX_full[%*ph]\n",
			4, (u8 *)tx_buf,
			4, (u8 *)rx_buf,
			(int)size, (u8 *)tc6->spi_ctrl_rx_buf);
		return -EPROTO;
	}
...
static int oa_tc6_mdiobus_read(struct mii_bus *bus, int addr, int regnum)
{
	...
	if (regnum & MII_ADDR_C45) {
		ret = oa_tc6_get_phy_c45_mms((regnum >> MII_DEVADDR_C45_SHIFT) & 0x1f);
		if (ret < 0)
			return ret;

		reg = (ret << 16) | (regnum & 0xffff);
	} else {
		reg = OA_TC6_PHY_STD_REG_ADDR_BASE |
		      (regnum & OA_TC6_PHY_STD_REG_ADDR_MASK);
	}

	ret = oa_tc6_read_register(tc6, reg, &regval);
	if (ret)
		return ret;
				...
				u16 val)
{
	...
	if (regnum & MII_ADDR_C45) {
		mms = oa_tc6_get_phy_c45_mms((regnum >> MII_DEVADDR_C45_SHIFT) & 0x1f);
		if (mms < 0)
			return mms;

		reg = (mms << 16) | (regnum & 0xffff);
	} else {
		reg = OA_TC6_PHY_STD_REG_ADDR_BASE |
		      (regnum & OA_TC6_PHY_STD_REG_ADDR_MASK);
	}

	return oa_tc6_write_register(tc6, reg, val);
}
...
static int oa_tc6_mdiobus_register(struct oa_tc6 *tc6)
{
	...
	/* The LAN865x internal PHY is exposed through the MAC-PHY itself and the
	 * current backport ignores the MDIO address argument in the bus accessors.
	 * Restrict phylib probing to address 0 so it does not discover the same
	 * internal PHY at every address on the synthetic bus.
	 */
	tc6->mdiobus->phy_mask = ~BIT(0);
...
static int oa_tc6_sw_reset_macphy(struct oa_tc6 *tc6)
{
	...
	/* Confirm SPI reads work before touching the chip reset register.
	 * If this read fails the SPI link itself is broken (signal integrity,
	 * wrong mode, etc.) and the problem is unrelated to the reset sequence.
	 */
	if (oa_tc6_read_register(tc6, OA_TC6_REG_STATUS0, &regval) == 0)
		dev_info(&tc6->spi->dev,
			 "pre-reset STATUS0=0x%08x (RESETC=%d)\n",
			 regval, !!(regval & STATUS0_RESETC));
	else
		dev_err(&tc6->spi->dev,
			 "pre-reset STATUS0 read FAILED – SPI link broken before SWRESET\n");
	regval = RESET_SWRESET;

	/* The LAN8651 begins hardware reset immediately upon receiving SWRESET,
	 * before it can complete the SPI echo transaction. Bypass the echo check
	 * by preparing and sending the SPI frame directly without calling
	 * oa_tc6_write_register (which would fail with -EPROTO on the echo).
	 */
	...
	/* The LAN865x takes considerably longer than the OA-TC6 minimum
	 * TRSTmin (500 µs) to re-establish SPI communication after a software
	 * reset.  On a CONFIG_HZ=100 kernel usleep_range(1000) rounds up to a
	 * full 10 ms tick, so readx_poll_timeout with a 1 ms sleep times out
	 * in wall-clock ~120 ms before the chip is ready.  Use a 100 ms flat
	 * wait followed by a manual retry loop so we tolerate any reset
	 * duration up to 3 s.  During the reset window the chip drives MISO
	 * low so reads return -EPROTO; treat that as "not yet ready".
	 */
	msleep(100);
	ret = -ENODEV;
	for (i = 0; i < 300; i++) {
		if (oa_tc6_read_register(tc6, OA_TC6_REG_STATUS0, &regval) == 0) {
			if (regval & STATUS0_RESETC) {
				ret = 0;
				break;
			}
		}
	...
	if (ret) {
		dev_err(&tc6->spi->dev,
			"MAC-PHY reset did not complete within 3 s\n");
		return ret;
	}
