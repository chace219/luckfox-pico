						 ...
						 enum dw_pcie_as_type as_type)
{
	...
	u32 offset = DEFAULT_DBI_ATU_OFFSET + PCIE_GET_ATU_INB_UNR_REG_OFFSET(index);
				    ...
				    dma_addr_t cpu_addr)
{
	...
	ret = rockchip_pcie_prog_inbound_atu_unroll(rockchip, 0, free_win, bar, cpu_addr, DW_PCIE_AS_MEM);
	if (ret < 0) {
