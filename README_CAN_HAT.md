...
**HAT:** CAN FD HAT (MCP2518FD CAN FD controller + CAN FD transceiver)  
**CAN oscillator:** 40 MHz (on-board crystal)  
**SPI clock:** 10 MHz (`spi-max-frequency = <10000000>`) — the MCP2518FD allows up to osc/2 = 20 MHz, but 10 MHz is used for signal-integrity margin (the FD driver is CRC-checked and 20 MHz over header/jumper wiring triggers CRC read errors)  
...
Both the MCP2518FD CAN controller and the LAN8651 Single-Pair Ethernet adapter are attached to the
same SPI0_M0 bus. They are distinguished by separate hardware chip-selects:

| Device   | SPI CS            | Kernel pin group  | `reg` in DTS |
|----------|-------------------|-------------------|--------------|
| MCP2518FD  | SPI0_CS0 GPIO1_C0 | `spi0m0_cs0`      | `<0>`        |
...
- Added a `fixed-clock` node (`mcp2518fd_osc`) at 40 MHz in the root node.
- Added `mcp2518fd_int_pin` pinctrl entry for the IRQ line on GPIO1_C7.
- Added `mcp2518fd: can@0` SPI device node under `&spi0` with `compatible = "microchip,mcp2518fd"` and **`spi-max-frequency = <10000000>`** (10 MHz).
...
```sh
# MCP2518FD SPI device must appear
ls /sys/bus/spi/devices/
# Expected: spi0.0  spi0.1  (mcp2518fd and lan8651)

cat /sys/bus/spi/devices/spi0.0/modalias
# Expected: spi:mcp2518fd
...
dmesg | grep -i mcp251
# Expected: mcp251xfd spi0.0 can0: MCP2518FD successfully initialized.
#           mcp251xfd spi0.0 can0: bit-timing not yet defined  ← normal, configure below
```
...
Common nominal (arbitration) bitrates supported by MCP2518FD at 40 MHz oscillator:
...
| 1 Mbit/s  | Max classic CAN / CAN FD arbitration phase |

Unlike the classic MCP2515, the MCP2518FD is a **CAN FD** controller: in CAN FD mode
it supports a faster data-phase bitrate (up to ~8 Mbit/s) via bit-rate switching (BRS).
Configure FD with, e.g.:
...
The MCP2518FD supports an internal loopback mode that echoes every transmitted frame back to the
...
# PASS means MCP2518FD SPI communication and interrupt are working.
...
=== Kernel: MCP2518FD driver probe ===
  [INFO] [  11.687] mcp251xfd spi0.0 can0: MCP2518FD successfully initialized.
  [PASS] Driver probed without errors

=== SPI device enumeration ===
  [PASS] SPI device spi0.0 exists
  [INFO] modalias = spi:mcp2518fd
  [PASS] modalias matches MCP2518FD

=== CAN network interface: can0 ===
  [PASS] Interface can0 exists
  [INFO] can clock   = 40000000 Hz
  [PASS] CAN clock frequency matches expected 40 MHz oscillator
...
Check dmesg for MCP2518FD probe errors:
...
| `mcp251xfd ... CRC read error` / oscillator timeout on probe | Oscillator absent or wrong frequency (check 40 MHz crystal), or SPI clock too high; the FD driver is CRC-checked and fails fast on bad SPI |
| No MCP2518FD messages at all | Kernel built without `CONFIG_CAN_MCP251XFD=y`, or DTS node missing |
